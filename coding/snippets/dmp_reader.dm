dmp_reader{
	/*
		...
		Released February 10th, 2007.

		defines the object /dmp_reader
			- provides one public method, load_map()
		...
		settings in order to fit the new map file, and loads the map onto new z levels; it will not
		shrink the world.

		Note that all type paths specified in the .dmp file must exist in the world's code. Also,
		much like DS, the /dmp_reader trusts that files to be loaded are in fact valid .dmp files.
		Errors in the .dmp format will cause runtime errors.
		*/
	var{
		quote = "\""
		}
	verb{
		load_map(var/dmp_file as file){
			var/z_offset = world.maxz
			var/quote = ascii2text(34)
			var/tfile = file2text(dmp_file)
		...
		parse_grid(var/model as text,var/xcrd as num,var/ycrd as num,var/zcrd as num){
			/*Method parse_grid()
				- Accepts a text string containing a comma separated list of type paths of the
					same construction as those contained in a .dmp file, and instantiates them.
				*/
			...
			for(var/dpos=1;dpos!=0;dpos=findtext(model,",",dpos,0)+1){
				...
				//Begin Instanciation
				var/atom/instance
				var/dmp_writer/preloader/_preloader// = new /dmp_writer/preloader(fields)
...
atom/New(atom/loc, dmp_writer/preloader/_dmp_preloader){
	if(istype(_dmp_preloader, /dmp_writer/preloader)){
		_dmp_preloader.load(src)
		}
	. = ..()
	}
dmp_writer{
